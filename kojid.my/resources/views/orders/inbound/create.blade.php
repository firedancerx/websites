@extends('layouts.app')
@section('title', __('orders.new_inbound'))

@section('content')
<div class="max-w-4xl mx-auto space-y-6">
    <div class="page-header">
        <div>
            <h1 class="page-title">{{ __('orders.new_inbound') }}</h1>
            <p class="page-subtitle">{{ __('orders.inbound_create_subtitle') }}</p>
        </div>
        <a href="{{ route('orders.inbound.index') }}" class="btn-secondary btn-sm">← {{ __('app.back') }}</a>
    </div>

    <form method="POST" action="{{ route('orders.inbound.store') }}" x-data="orderForm()">
        @csrf

        {{-- Supplier Selection --}}
        <div class="card">
            <div class="card-header">
                <h2 class="font-semibold text-slate-800">{{ __('orders.supplier_details') }}</h2>
            </div>
            <div class="card-body grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    <label class="form-label">{{ __('orders.supplier') }} *</label>
                    <select name="entity_id" class="form-select" required
                            @change="onSupplierChange($event)">
                        <option value="">{{ __('orders.select_supplier') }}</option>
                        @foreach($suppliers as $s)
                            <option value="{{ $s->id }}"
                                    data-rate="{{ $s->deposit_rate_override ?? $s->tenant->getDefaultDepositRate() }}"
                                    data-terms="{{ $s->credit_terms }}"
                                    {{ old('entity_id') == $s->id ? 'selected' : '' }}>
                                {{ $s->name }} ({{ $s->ssm_number }})
                                @if(!$s->isVerified()) ⚠ {{ __('entities.unverified') }} @endif
                            </option>
                        @endforeach
                    </select>
        ...
        <div class="flex justify-end gap-3">
            <a href="{{ route('orders.inbound.index') }}" class="btn-secondary">{{ __('app.cancel') }}</a>
